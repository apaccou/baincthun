...
$files = array(
    'vendor/uikit.min.css',
		'vendor/components/search.min.css',
    //'vendor/lightbox.css',
    'css3_mixins.css',
    'application.css',
);
...
if (! $recache && isset ( $_SERVER ['If-Modified-Since'] ) && strtotime ( $_SERVER ['If-Modified-Since'] ) >= $time) {
	...
	if ($recache) {
		...
		$less = new lessc();
        $less->setFormatter("classic");//désactiver en production
        //$less->setFormatter("compressed");//activer en production
		$css = $less->compile($css);
		//$css = minifyCSS($css);//activer en production pour compresser encore plus
